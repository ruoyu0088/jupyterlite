...
expr_patterns = dict(
    create_line=f"{line()}",
    create_arc=rf"{circle('arc')}{eq}{point('center')}\s+{point('start')}\s+{point('end')}",
    ...
    create_circle=rf"{circle()}{eq}{point()}\s+{number()}",
    circle_radius=rf"r\s+{circle()}{eq}{number()}",
    circle_diameter=rf"d\s+{circle()}{eq}{number()}",
...
class BokehPlot:
    ...
    def create_figure(self):
        ...
        self.render_points = fig.scatter(
